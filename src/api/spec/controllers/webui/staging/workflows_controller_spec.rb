...
RSpec.describe Webui::Staging::WorkflowsController do
  ...
  let(:staging_workflow) { create(:staging_workflow, project: project, managers: managers_group) }
  ...
  describe 'PUT #update' do
    ...
    context 'with a failing save for staging workflow' do
      before do
        staging_workflow
        allow_any_instance_of(Staging::Workflow).to receive(:save!).and_raise('any error')
        put :update, params: { id: staging_workflow, managers_title: other_managers_group.title }
      end
